						...
						<ul class="list-group">
							...
							<li class="list-group-item"><strong> <span class="text-dark"> Order Status  : </span> </strong>
								...
								@elseif ($order->order_status === 'Processing')						 
									<span class="badge badge-pill" style="background: #05cff3;">{{ $order->order_status }}</span>
									@else
									<span class="badge badge-pill" style="background: #03994b;">{{ $order->order_status }}</span> 
								@endif
							...
							@if($order->order_status === 'Pending')
							<a href="{{ route('pending-processing', $order->id) }}" class="btn btn-block btn-success"> Processing your Order</a>

							@elseif($order->order_status === 'Processing')
								<a href="{{ route('processing-complete', $order->id) }}" class="btn btn-block btn-success">Order Complete</a>
							@endif
