...
class OrderList extends Component {
  ...
  PostReview = () => {
    ...
    if(name.length===0){
        ...
        cogoToast.error("Comments can't be more then 150 character", {position:'top-right'});
   }
   else{
    ...
    if(response.data===1){
          ...
          cogoToast.error("Your Request not processed! Try Aagain", {position:'top-right'});
    }
    }).catch(error=>{
        cogoToast.error("Your Request not processed! Try Aagain", {position:'top-right'});
      });
    ...
    return (
      <Fragment>
        ...
        <Modal show={this.state.ReviewModal} onHide={this.ReviewModalClose}>
          ...
          <Modal.Footer>
            <Button variant="secondary" onClick={this.PostReview}>Post</Button>
