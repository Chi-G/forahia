...
class ProductDetails extends Component {
     ...
     addToCart = () => {
          ...
          if(isColor==="YES" && color.length===0){
               ...
               axios.post(AppURL.addToCart,MyFormData).then(response =>{
                    if(response.data===1){
                         cogoToast.success("Product Order Added Successfully",{position:'top-right'});
     ...
     orderNow = () => {
          ...
          if(isColor==="YES" && color.length===0){
               ...
               axios.post(AppURL.addToCart,MyFormData).then(response =>{
                    if(response.data===1){
                         cogoToast.success("Product Ordered Successfully",{position:'top-right'});
          ...
          if(!localStorage.getItem('token')){
             cogoToast.warn('Please You have to Login First',{position:'top-right'});
               ...
               axios.get(AppURL.AddFavourite(productCode,email)).then(response =>{
                    if(response.data===1){
                         cogoToast.success("Product added to Favourite successfully",{position:'top-right'});
